...
def update_variable(q, candidates_list, n_experiment_index, master_data, experiment_index, modified_X_baseline):
    if len(candidates_list) == 0:
        ...
        return q, n_experiment_index, new_choices, None
...
## index
experiment_index   = master_all_data[master_all_data["test"] == True ]["Index"].tolist()
n_experiment_index = master_all_data[master_all_data["test"] == False]["Index"].tolist()

## X_baseline, choices
X_baseline = torch.tensor(  experiment_index, dtype=float).unsqueeze(-1)
choices    = torch.tensor(n_experiment_index, dtype=float).unsqueeze(-1)
